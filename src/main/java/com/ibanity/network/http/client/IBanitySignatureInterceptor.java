...
import com.ibanity.exceptions.DigestException;
import com.ibanity.exceptions.InvalidDefaultHttpHeaderForSignatureException;
import com.ibanity.exceptions.SignatureException;
...
public class IBanitySignatureInterceptor implements HttpRequestInterceptor {
    ...
    private void setDefaultHttpHeaderValues(HttpRequest httpRequest, byte[] body) throws DigestException {
    ...
    private void setDigestHeader(HttpRequest httpRequest, byte[] body) throws DigestException {
        String digestValue;
        try {
            digestValue = Base64.getEncoder().encodeToString(MessageDigest.getInstance(DIGEST_ALGORITHM).digest(body));
            ...
            throw new DigestException(errorMessage, e);
        }
    }

    private void setSignatureHeader(HttpRequest httpRequest, byte[] body) throws SignatureException {
        HttpRequestWrapper requestWrapper = (HttpRequestWrapper) httpRequest;
    ...
    private String generateSignature(HttpRequestWrapper requestWrapper, String signatureHeaders) throws SignatureException {
        try {
    ...
    private String getSignatureAlgorithm(boolean forAlgorithmHeader) throws SignatureException {
